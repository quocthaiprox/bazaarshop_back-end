...
class ProductController {
  ...
  async getSomeProduct(req, res) {
    ...
    try {
      let products = [];
      let maxPrice = (await Product.findOne().sort({ price: "desc" }).exec())
        .price;
      let minPrice = (await Product.findOne().sort({ price: "asc" }).exec())
        .price;
      ...
      products = (
        await Product.find(whereObj).sort(sortObj).limit(endIndex).exec()
      ).slice(startIndex);

      return res
        .status(200)
        .json({ success: true, products, pagination, maxPrice, minPrice });
    } catch (err) {
  ...
  async createProduct(req, res) {
    ...
    if (!title || !price || !image)
      return res.status(400).json({
        success: false,
        message: "Title, price, image is required",
      });
